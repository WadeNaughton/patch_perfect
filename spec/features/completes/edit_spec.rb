...
RSpec.describe 'Completed hike show page page' do
    ...
    it "has link to add a title" do
        visit("/users/#{@user.id}/hikes/#{@hike2.id}/complete")
        expect(page).to have_link("Add a Title")
        click_link("Add a Title")
        expect(current_path).to eq("/users/#{@user.id}/hikes/#{@hike2.id}/complete/edit")
